...
class Grapher:
    ...
    def relative_distance(self, export_document_graph=False):
        ...
        if export_document_graph:
            for idx, row in df.iterrows():
                ...
                if np.isnan(row['destination_x_vert']) == False:
                    ...
                    text_coordinates = (
                    int((row['destination_x_vert'] + source_x) / 2), int((row['destination_y_vert'] + source_y) / 2))
                ...
                if np.isnan(row['destination_x_hori']) == False:
                    ...
                    text_coordinates = (
                    int((row['destination_x_hori'] + source_x) / 2), int((row['destination_y_hori'] + source_y) / 2))
                    cv2.putText(img, text, text_coordinates, cv2.FONT_HERSHEY_DUPLEX, 0.4, (255, 0, 0), 1)
        ...
        df.drop(['destination_x_hori', 'destination_y_hori', 'destination_y_vert', 'destination_x_vert'], axis=1,
                inplace=True)
        print(df.columns, 'muhannad')
        self.get_text_features(df)
        print(df.columns, 'muhannad2222')
        return df
